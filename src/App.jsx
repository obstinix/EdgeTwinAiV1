  ...
  const renderMachineNode = (mid, cx, cy, name) => {
    ...
    return (
      ...
      >
        ...
        <g stroke={strokeColor} strokeWidth={isSelected ? 1.8 : 1.2} fill="none" opacity={isSelected ? 1 : 0.4} filter={glowFilter}>
          {/* Top-Left */}
        ...
        <rect x={cx - 22} y={cy - 52} width="44" height="18" rx="4" fill="#ffffff" stroke={strokeColor} strokeWidth="1" opacity="0.9" />
        ...
        {faultLabel && (
          <text
            ...
            y={cy + 66}
            textAnchor="middle"
            fill={status === "critical" ? "#e11d48" : "#d97706"}
            fontSize="8"
            fontWeight="800"
...
<body>
  ...
  return (
    <div className="flex h-screen overflow-hidden text-slate-800 font-sans">
      {/* SIDEBAR NAVIGATION */}
      <aside className={`${sidebarOpen ? 'w-64' : 'w-52'} bg-slate-50 border-r border-slate-200 flex flex-col shrink-0 overflow-y-auto transition-all duration-300`}>

        {/* Logo + Toggle */}
        <div className="border-b border-slate-200 flex items-center px-3.5 py-4.5 gap-3 justify-between">
          <div className="flex items-center gap-2 min-w-0">
            <div className="relative shrink-0">
              <Cpu className="w-5.5 h-5.5 text-[#0061FF]" />
              <span className={`absolute -bottom-0.5 -right-0.5 w-2 h-2 rounded-full border border-slate-50 ${wsConnected ? 'bg-emerald-500' : 'bg-rose-500'}`}></span>
            </div>
            <div className="min-w-0">
              <h1 className="text-sm font-bold font-hud tracking-widest text-slate-900 whitespace-nowrap">EDGETWIN AI</h1>
              <p className="text-[8px] text-[#0061FF] uppercase tracking-widest font-mono font-bold whitespace-nowrap">DECISION INTELLIGENCE</p>
            </div>
          </div>
          <button
            onClick={() => setSidebarOpen(prev => !prev)}
            className="text-slate-400 hover:text-slate-800 transition p-1 rounded hover:bg-slate-200/50 shrink-0"
        ...
        <nav className="px-3 pt-3 pb-2 space-y-1">
          {[
            ...
            <button
              ...
              className={`relative w-full flex items-center gap-3 px-3.5 py-2.5 rounded-lg text-xs font-semibold tracking-wider uppercase font-display transition-all duration-200 active-press ${activeTab === tab ? "bg-[#0061FF]/8 text-[#0061FF] nav-indicator-active" : "text-slate-600 hover:text-slate-900 hover:bg-slate-200/40"}`}
            >
              {icon}
        ...
        <div className="mx-3 mb-3 rounded-xl bg-slate-100/50 border border-slate-200">
          <button
            onClick={() => setSimLabOpen(prev => !prev)}
            className="w-full flex items-center justify-between px-3 py-3 text-xs font-semibold text-slate-700 font-display"
          >
            <span className="flex items-center gap-2">
              <ShieldAlert className="w-4 h-4 shrink-0 text-slate-500" />
              <span className="whitespace-nowrap">AI SCENARIO SIMULATOR</span>
            </span>
            <span className={`transition-transform duration-200 text-slate-500 text-base leading-none ${simLabOpen ? 'rotate-180' : ''}`}>▾</span>
          </button>

          {simLabOpen && (
            <div className="px-3 pb-3 space-y-2 text-xs border-t border-slate-250">
              <div className="pt-2">
                <label className="text-slate-600 font-semibold block mb-1">Target Asset</label>
                <select
                  value={anomalyMachine}
                  onChange={(e) => { setAnomalyMachine(e.target.value); setSelectedMachine(e.target.value); }}
                  className="w-full bg-white border border-slate-200 rounded px-2 py-1.5 text-slate-800 outline-none focus:border-[#0061FF]/40"
                >
              ...
              <div>
                <label className="text-slate-600 font-semibold block mb-1">Anomaly Type</label>
                <select
                  value={anomalyType}
              ...
              <div className="flex gap-2 pt-1">
                <button onClick={injectAnomaly} className="flex-1 bg-rose-600 hover:bg-rose-500 active-press text-white font-medium py-1.5 rounded flex items-center justify-center gap-1 transition text-[11px]">
                  <Play className="w-3 h-3 fill-white" /><span>Simulate Failure</span>
                </button>
                <button onClick={resetAllMachines} className="flex-1 bg-emerald-600 hover:bg-emerald-500 active-press text-white border border-emerald-500/20 font-medium py-1.5 rounded flex items-center justify-center gap-1 transition text-[11px]">
      ...
      <main className="flex-1 flex flex-col bg-[#f1f5f9] overflow-hidden">
        
        {/* TOP STATUS BAR */}
        <header className="h-16 border-b border-slate-200 bg-white/95 px-8 flex items-center justify-between shrink-0 backdrop-blur-md">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-baseline gap-1.5">
                  ...
                  <span className={`font-bold text-sm font-mono status-transition ${factoryHealthPct >= 85 ? 'text-emerald-600' : factoryHealthPct >= 60 ? 'text-amber-600' : 'text-rose-600'}`}>
                    {factoryHealthPct.toFixed(1)}%
                  </span>
                </div>
                <p className="text-[9px] text-slate-500 font-mono uppercase tracking-widest leading-none">Overall Operational Availability</p>
              </div>
            </div>
          ...
          <div className="flex items-center gap-4">
            <button 
              ...
              className="bg-slate-100 hover:bg-slate-200 px-3.5 py-1.5 rounded-lg text-xs font-semibold text-slate-700 flex items-center gap-1.5 transition active-press"
            >
            ...
            <button 
              ...
              className="bg-[#0061FF] hover:bg-[#0052D9] active:scale-95 px-3.5 py-1.5 rounded-lg text-xs font-semibold text-white flex items-center gap-1.5 transition active-press"
            >
